...
# %% #Train Test Set
df = pd.read_excel('../Data/MultiDonor_Fluo_UT.xlsx').drop(['Index'], axis=1)
label_group = list(set(np.around(df.RB_IDO.to_numpy(),2)))
# %%Initialize models - grid search
rfr = RandomForestRegressor(n_estimators=33, max_depth=3, random_state=42)
gbr = GradientBoostingRegressor(n_estimators=9, learning_rate=0.1, max_depth=3,random_state=42)
svr = SVR(C= 1, epsilon=1, kernel='rbf')
mlpr = MLPRegressor(activation='tanh', alpha= 251,
 hidden_layer_sizes= (50, 50, 50),
 learning_rate= 'constant',
 max_iter= 400,
 solver= 'sgd')
svrl = SVR(C= 1, epsilon=0.3, kernel='linear')
lasso = Lasso(alpha=1.220909090909091)
# %%
...
df_score.to_excel(r'../Data/ModelCVScore_Fluo_UT.xlsx')
# %%
avg_result = {}
for scoring_name, scoring_method in scoring_methods.items():
    try:
