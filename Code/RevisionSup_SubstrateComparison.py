...
df = pd.read_excel('../Data/RevisionSup_Cell.xlsx', sheet_name="Substrate2")
...
df_s.columns=['Condition', 'ImageNumber', 'ObjectNumber', 'AreaShape_Area',
       ...
       'Intensity_IntegratedIntensityEdge_RescaleIntensityCell',
...
# Remove rows with Condition == "ITO"
df_filtered = df_s[df_s['Condition'] != "MEM_ITO"]
df_balance = df_filtered.groupby('Condition', group_keys=False).apply(lambda x: x.sample(n=260, random_state=42)).reset_index(drop=True)
# %%Smooth data
...
condition_styles = {
    "MEM_dish": {'color': '#e346db', 'marker': 'o', 'size': 60, 'label': 'Glass Bottom Dish'},
    "MEM_ibidi": {'color': '#a9d943', 'marker': 'o', 'size': 60, 'label': 'ibidi uSlide'},
    # "MEM_ITO": {'color': '#6bd3f0', 'marker': 'o', 'size': 60, 'label': 'ITO Slide'},
    "Plastic": {'color': '#f09c6b', 'marker': 'o','size': 60, 'label': 'Plastic Plate'},
}
plt.figure(figsize=(7,7))
for Condition, style in condition_styles.items():
    data = pca_df.query(f'Condition == "{Condition}"')
    plt.scatter(
        data['PC1'],           # x-axis data (Labels)
        data['PC2'],      # y-axis data (Predictions)
...
plt.title('PCA of DPC Features: Plate Sytle\n', fontsize=20)
legend = plt.legend(
    bbox_to_anchor=(0.02, 0.98),  loc='upper left', 
    fontsize=20, borderaxespad=0
)
...
for handle in legend.legendHandles:
    handle._sizes = [200]  
plt.tight_layout()
plt.savefig('../Plot/RevisionSup/RB183_SubstrateComparison3_DPCFeatures.svg', format="svg",dpi=300,bbox_inches='tight') 
...
plt.savefig('../Plot/RevisionSup/RB183_SubstrateComparison3_DPCFeatures_PC1Importance.svg', format="svg",dpi=300,bbox_inches='tight') 
# %%
...
ax.set_ylabel(" ", fontsize = 30)
ax.tick_params(labelsize = 10)
plt.savefig('../Plot/RevisionSup/RB183_SubstrateComparison3_DPCFeatures_PC2Importance.svg', format="svg",dpi=300,bbox_inches='tight') 
# %%Screen out the cell shape
Morph = [col for col in df_RM.columns if col.startswith('AreaShape_')]

principal_components2 = pca.fit_transform(df_RM[Morph])
explained_var2 = pca.explained_variance_ratio_ * 100  # in %

pca_df2 = pd.DataFrame(data=principal_components2, columns=['PC1', 'PC2'])
pca_df2['Condition'] = df_RM['Condition']

plt.figure(figsize=(7,7))
# colors = ["#6bd3f0", "#e346db", "#a9d943"]
# sns.scatterplot(data=pca_df, x='PC1', y='PC2', hue='Condition', s=60, palette = sns.color_palette(colors))
for Condition, style in condition_styles.items():
    data = pca_df2.query(f'Condition == "{Condition}"')
    plt.scatter(
        data['PC1'],           # x-axis data (Labels)
        data['PC2'],      # y-axis data (Predictions)
        c=style['color'],              # color from dictionary
...
plt.ylabel(f'PC2 ({explained_var[1]:.1f}% Variance)', fontsize=30)
plt.tick_params(labelsize = 25) 
plt.title('PCA of Morph Features: culture plates\n', fontsize=20)
legend=plt.legend(fontsize=20,)
# Resize legend marker points
for handle in legend.legendHandles:
    handle._sizes = [200]  
plt.tight_layout()
plt.savefig('../Plot/RevisionSUp/RB183_SubstrateComparison3_MorphFeatures.svg', format="svg",dpi=300,bbox_inches='tight') 
# %%
...
ax.set_ylabel(" ", fontsize = 30)
ax.tick_params(labelsize = 10)
plt.savefig('../Plot/RevisionSup/RB183_SubstrateComparison3_MorphFeatures_PC1Importance.svg', format="svg",dpi=300,bbox_inches='tight') 
# %%
PC2Loading = PCLoading.sort_values(['PC2']).reset_index(drop=True)
plt.figure(figsize=(6,16))
...
ax.set_ylabel(" ", fontsize = 30)
ax.tick_params(labelsize = 10)
# plt.savefig('../Plot/RevisionSup/RB183_SubstrateComparison3_MorphFeatures_PC2Importance.svg', format="svg",dpi=300,bbox_inches='tight') 
